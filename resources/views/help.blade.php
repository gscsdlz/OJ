...
Queuing
你的程序，系统已经收到。正在redis队列中排队，<span class="text-danger">最大排队时间为3S</span>
Rejudging
由于题目数据更改或者其他原因，你提交的代码正在重判，一般重判后排名会出现变化。
...
你的程序输出了太多的东西。
Online Judge规定提交的程序在运行的时候最多只能输出比标准答案多两倍的内容，如果你输出太多，将导致此错误。
...
你的提交被管理员判断为作弊提交，一旦被判为作弊提交，你对该题的所有提交，均会判为作弊。
所有选手每一个正确的提交，均会接受代码查重程序的检查，会标记出相似度50%以上的代码。
管理员检查后确认是否作弊，一旦发现作弊，取消比赛资格，并且不再计入排名。
请大家诚信比赛。
</pre>
            <hr/>
...
6.请不要在代码中使用<span class="text-danger">system("pause");</span>
7.一份标准的Java代码如下
<code>
public class Main {
...
9.Online Judge的C/C++编译选项如下所示
gcc Main.c -o Main -fon-asm -Wall -lm --staic -std=c99 -DONLINE_JUDGE
g++ Main.cc -o Main -fon-asm -Wall -lm --staic -std=c++11 -DONLINE_JUDGE
</code>
</pre>
            <hr/>
...
一般情况下，C语言运行时I/O函数比C++的效率高一些，所以处理大数据量的时候，建议使用<span class="text-danger">scanf/printf</span>组合。
            ...
            <pre>
1.在用户界面且处于登录的情况下，双击自己的头像即可修改，上传的图片大小在<span class="text-danger">2M</span>之内
...
5.比赛过程中，点击排名可以按小组查看排名，<span class="text-danger">排名页面每5分钟更新一次。请不要频繁刷新页面</span>
6.用户每次登陆默认时长为6小时，不用担心比赛过程中重新登陆。
7.Online Judge的大部分核心功能依赖Javascript，请<span class="text-danger">不要禁用Javascript</span>。
8.<span class="text-danger">已经结束的比赛，不允许提交代码</span>，但结束比赛以后所有题目会显示在题目列表中，方便大家回顾。
9.Online Judge不允许频繁刷新，两次刷新之间的间隔在1秒以上。
</pre>
            <hr/>
        </div>
    </div>
